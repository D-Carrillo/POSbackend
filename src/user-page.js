...
const UserPage = () => {
    ...
    const fetchCustomerReports = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get(`${apiUrl}/api/reports/${period}/${user.id}`, {
                headers: {
    ...
    const fetchTransactions = async () => {
        if (!user?.id) return;
        try {
            const response = await axios.get(`${apiUrl}/api/userTransactions/${user.id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
